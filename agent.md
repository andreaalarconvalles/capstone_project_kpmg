...
Keep the scripted prompt experience polished as the demo fallback. Auto Agent is the default entry point: the UI/backend should infer the best specialist from the user prompt, then route to Host Revenue, Gentrification Early Warning, STR Financial Crime Detection, Tourism Demand Forecast, or Market Entry Advisor. Manual specialist selection remains an advanced override. Typed custom prompts should use grounded live data when Vertex authentication is configured, with the workflow folded by default, no internal quality score displayed, no raw snake_case labels, no repeated KPI cards, and readable sectioned answers.
...
- [2026-06-13] Session 53: Added Auto Agent routing as the default ARIA interaction mode. The frontend now resolves typed prompts to the best specialist before scripted/live execution, New Chat returns to Auto Agent, and `/api/chat` includes a matching server-side router for direct API calls and live Vertex analysis.
