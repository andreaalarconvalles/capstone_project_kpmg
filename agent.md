...
The `/api/chat` handler includes a 3-lane scope router. Every prompt is classified before it reaches the analytics pipeline: `in_scope` (Paris/Athens STR analytics) runs the full pipeline; `other_city` and `general` are answered directly with lightweight system prompts. The deterministic gate (`isInScopeDomain`) fast-paths obvious ARIA prompts without calling the LLM router.
