...
The `/api/chat` handler now includes a 3-lane scope router. Every prompt is classified before it reaches the analytics pipeline: `in_scope` (Paris/Athens STR analytics) runs the full pipeline with Gemini Pro; `other_city` and `general` are answered by Gemini Flash with lightweight system prompts. The deterministic gate (`isInScopeDomain`) fast-paths obvious ARIA prompts without calling the LLM router. On router failure the fallback is `general`, not `in_scope`, to prevent the analytics pipeline from running on off-topic prompts. All responses now include a `scope` field (`"in_scope"`, `"general"`, or `"other_city"`) for frontend diagnostics.
