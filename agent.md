...
- [2026-06-26] Scope-routing hardening (Sessions 71-75): added a 3-lane scope router so off-topic prompts stop receiving the investor analytics template. Fixed the Gemini 2.5 empty-label bug (thinkingBudget 0 + 512 output tokens, Pro-model retry, `parseScopeLabel()` no longer defaults to `in_scope`, unresolved scope maps to `general`). Tightened the upstream `isInScopeDomain()` gate to high-precision (city + real-estate/STR intent, unambiguous ARIA terms, compliance, or both cities) with exact city aliases, so "weather in Paris", "history of Athens", "parts of a car engine", "who is Aria", and "is KPMG hiring" no longer trigger analytics. Renamed UI themes (Light / Ocean / Dark). Verified offline with `scripts/scope_smoke_test.mjs` (24/24); routing-logic quality estimated ~97/100.
...
The active UI/backend code lives in `Stage 7 - UI Interface/vercel_vite_app/`. The custom-prompt path is `api/chat.js` plus `api/analytics-pipeline.js`: it fetches committed GitHub CSV outputs, computes deterministic analytics, calls Vertex AI server-side, and returns structured `answer`, `kpis`, `visualizations`, `details`, and `sources`. Gemini 3.5 Flash is the default selected model for every fresh session (set via `useState` in `aria-main.jsx` and the `default` flag in `aria-data.jsx`; routed to the `global` endpoint); the picker also exposes Gemini 2.5 Pro / 2.5 Flash / 2.5 Flash-Lite, and Claude Sonnet 4.6 / Opus 4.7. Note: the backend `DEFAULT_MODEL` (`gemini-2.5-pro`) is the unavailable-model FALLBACK and is deliberately kept on the default `europe-west1` region — it is separate from the picker's default selection. Gemini models use the Google `generateContent` route; Claude models use the Vertex AI Anthropic partner `rawPredict` route. Some Google models are served only from the multi-region `global` endpoint, not the configured regional one — `api/chat.js` has `MODEL_LOCATION_OVERRIDES` (e.g. `gemini-3.5-flash` -> `global`) and `callVertexModelWithFallback()` routes each model to its required location while the Gemini fallback stays on the default region.

Model availability (probed against `capstoneprojectkpmg` with a service-account token, 2026-06-26/27): in `europe-west1` only `gemini-2.5-pro`, `gemini-2.5-flash`, and `gemini-2.5-flash-lite` return 200. `gemini-3.5-flash` (and `gemini-flash-latest`) return 404 in `europe-west1` but 200 in the `global` region, which is why they are routed there via `MODEL_LOCATION_OVERRIDES`. `gemini-3.5-flash` is a thinking model (uses ~400+ "thoughts" tokens before its answer); the app's 2600 `maxOutputTokens` is ample, but very low token budgets return empty text with `finishReason: MAX_TOKENS`. `gemini-2.0-flash`, `gemini-3.x` pro variants, and `gemini-pro-latest` are 404 everywhere for this project. The Claude IDs `claude-sonnet-4-6` and `claude-opus-4-7` are CORRECT and enabled (they return 429 "Quota exceeded", not 404) — they fail only because the project has no Anthropic model quota allocated. To make Claude actually run, request quota in GCP for the Anthropic base models (Vertex AI -> Quotas, filter by the model) and wait for approval; no code change is needed. Meanwhile `api/chat.js` `callVertexModelWithFallback()` treats both 404 and 429 as "unavailable" and retries with the default Gemini model, prepending a transparent note, so the picker never hard-fails. IMPORTANT Vercel detail: env vars (service-account creds, project id) are set for the Production environment ONLY — Preview deployments report `authConfigured:false` and cannot make live calls, so live testing must use the production (main) URL or a local run with the credential in `.env`.

The `/api/chat` handler includes a 3-lane scope router. Every prompt is classified before it reaches the analytics pipeline: `in_scope` (Paris/Athens STR analytics) runs the full pipeline; `general` is answered as a normal assistant WITH live web search; `other_city` keeps the "not trained on that market's data" deflection. The deterministic gate (`isInScopeDomain`) fast-paths obvious ARIA prompts without calling the LLM router.

Web search (general lane only): the `general` lane uses Gemini's built-in Google Search grounding (`tools: [{ googleSearch: {} }]` via `callGeminiVertexGrounded`). The model decides on its own when to search (time-sensitive questions: sports fixtures, news, prices, weather, "latest/next/upcoming"), answers static knowledge directly without searching, and synthesises results. `buildGeneralSystemPrompt()` injects the current UTC date/time so the model reasons about past vs future events correctly. Grounding works only on Gemini, so if a non-Gemini model (Claude) is selected the lane falls back to the default Gemini; if grounding errors it retries without search. Web sources from `groundingMetadata.groundingChunks` are appended as a markdown `Sources:` line, and `renderInline()` in `aria-ui.jsx` was extended to render `[title](url)` links. `other_city` and `in_scope` deliberately do NOT web-search (STR market analysis of unsupported cities stays a deflection).

Gate precision (the first decision — `isInScopeDomain()` in `analytics-pipeline.js`): a prompt that passes the gate skips the router and runs analytics directly, so the gate must be high-precision. It fast-paths ONLY on (1) unambiguous ARIA-internal terms (arrondissement, underpric, shap, xgboost, lightgbm, opportunity score, host revenue, loi le meur — deliberately NOT "aria"/"kpmg"/"prophet", which are common words routed for identity/company handling), (2) compliance intent, (3) both Paris and Athens named together, or (4) a single supported city paired with real-estate/STR/market intent (`STR_REALESTATE_INTENT`). A bare city mention alone no longer fast-paths, so "weather in Paris", "history of Athens", and "is it safe to walk in Athens" defer to the router and are answered generally. The gate uses exact city aliases (fuzzy:false) so near-miss tokens like "parts"~"paris" cannot fabricate a city mention. Verify with `scripts/scope_smoke_test.mjs` (24 offline gate assertions).

Router robustness (critical — this is what keeps math/haiku/other-city prompts out of the analytics template):
- The router asks Gemini Flash first, then retries with the default Pro model if Flash is unavailable, errors, or returns an unparseable label. Both are Gemini `generateContent` calls; Claude models are skipped for routing.
- Flash runs with `thinkingConfig.thinkingBudget = 0` and `maxOutputTokens: 512`. Gemini 2.5 models otherwise spend the output budget on internal "thinking" and return an empty label — the original `maxOutputTokens: 8`/`20` caused exactly this, so every off-topic prompt fell through to the analytics pipeline.
- `parseScopeLabel()` returns `""` (not `"in_scope"`) for an empty/unparseable reply. The only safe default once `isInScopeDomain()` has already returned false is `general`, never `in_scope`.
- `resolveScope()` maps any failure or empty result to `general`. All responses include a `scope` field (`"in_scope" | "general" | "other_city"`) for the frontend, which swaps the reasoning-trace header to a 2-step "Scope Router → Assistant" trace on off-scope answers instead of the 6-agent analytics trace.
...
- [2026-06-23] Session 65: Added a reproducible local ChromaDB utility for the Phase 5 compliance corpus, documented the build/query/validate workflow, kept generated `rag/chroma_db/` local and gitignored, and validated registration, freeze, enforcement, and Loi Le Meur retrieval smoke tests.
- [2026-06-23] Session 66: Added the reproducibility layer: root analytical `requirements.txt`, frontend `package-lock.json`, schema contracts for committed output artifacts, `scripts/validate_outputs.py` as the one-command data/model validator, and a GitHub Actions output-contract validation job.
- [2026-06-23] Session 67: Cleared the previous verification caveats by upgrading Vite and Google Auth frontend dependencies to audit-clean versions, moving CI to Node 22, adding Vite/Rolldown vendor chunk splitting to remove the oversized bundle warning, and running local verification with PowerShell profile loading disabled.
- [2026-06-23] Session 68: Fixed ARIA compliance response routing so AMA, central Athens freeze/regularisation, enforcement, and Loi Le Meur prompts answer from RAG citation passages first, suppress unrelated investment visuals, include citation IDs in the final Sources line, and are covered by response-quality regression checks.
- [2026-06-23] Session 69: Polished ARIA live-answer formatting by replacing raw compliance snippets with curated source evidence, removing Markdown table leakage and awkward metric punctuation, preserving citation IDs, improving pricing/risk fallback text, and adding regression checks for the screenshot artifacts.
- [2026-06-24] Session 70: Clarified the RAG deployment status for reviewers, documenting that Vercel is wired to committed RAG handoff outputs while ChromaDB remains a local reproducibility utility, added a root `environment.yml`, and validated the committed serialized RAG corpus artifact.
- [2026-06-26] Session 71: Updated the Stage 7 sidebar profile avatar to use the IE University logo asset and replaced the Settings About text with a concise academic-transparency disclosure for generative-AI assistant use.
- [2026-06-27] Session 72: Changed the Stage 7 sidebar profile display name from the personal name to `Guest` in both the Vercel app source and prototype mirror.
- [2026-06-26] Session 71: Hardened the scope router in `api/chat.js` — changed router failure fallback from `in_scope` to `general`, tightened the classifier prompt with an explicit city-specificity rule and `other_city` examples, and raised `maxOutputTokens` from 8 to 20.
- [2026-06-26] Session 72: Renamed UI themes from Airbnb/KPMG/Dark to Light/Ocean/Dark in `aria-data.jsx`, swapped the Ocean palette icon from Landmark to Waves.
- [2026-06-26] Session 73: Six `api/chat.js` quality improvements: Flash model for general/other_city lanes, conditional server-time injection, concrete pivot offer in other_city prompt, router failure `console.error` logging, `scope` field in all API responses, and removed the hardcoded home-buying disclaimer from `addContextIfShort`.
- [2026-06-26] Session 74: Diagnosed from Lane 2/3 test screenshots that off-topic prompts (math, haiku, capital of Japan, Lisbon, Berlin vs Madrid, Barcelona) all ran the full Paris analytics pipeline. Root cause: Gemini 2.5 router spent its tiny `maxOutputTokens` budget on internal thinking and returned an empty label, which `parseScopeLabel()` defaulted to `in_scope`. Fixed by setting Flash `thinkingBudget: 0` + `maxOutputTokens: 512`, retrying the router with the known-good Pro model on failure/empty, making `parseScopeLabel()` return `""` instead of `in_scope`, and mapping unresolved scope to `general`. Reverted the off-scope handler from Flash back to the main Pro model for reliability. Frontend now shows a 2-step trace on off-scope answers.
- [2026-06-27] Session 84: Fixed follow-up context loss in the scope router. After an in-scope Athens/Paris analysis, a short follow-up like "show me a map as well" was being classified `general` (the Flash router ignored the conversation context even though the prompt already has a continuation rule). Added a deterministic `isInScopeFollowUp(prompt, messages)` safety net in `resolveScope`: if the new prompt is a visualisation / drill-down / back-reference follow-up (map, chart, "break it down", "those", "in more detail", etc.) AND a recent turn passes `isInScopeDomain`, it inherits `in_scope` without trusting the LLM. City-introducing words ("compare", "what about") are excluded so "compare Berlin and Madrid" still routes via the LLM. Validated 9/9 (incl. math/haiku/other-city follow-ups NOT forced in-scope); smoke test 24/24.
- [2026-06-27] Session 83: Added a prompt-enhancer. A floating "Enhance" button appears at the top-right of the composer once the input has >= 5 words; clicking it calls a new `mode: "enhance"` branch in `api/chat.js` (`enhanceUserPrompt()` -> shared `flashRewrite()` on gemini-2.5-flash) that rewrites the prompt to be clearer and more professional with LIGHT detail, anchored to ARIA's real domain, with no invented facts/dimensions. The improved prompt is shown in a non-destructive preview popup above the input with Apply / Dismiss (user choice). Refactored the voice `polishTranscript()` and the new `enhanceUserPrompt()` to share `flashRewrite()`. `renderInline()` link support and the web-search/voice features remain from sessions 81-82.
- [2026-06-27] Session 82: Added voice-dictation cleanup. While the mic is on, the raw Web Speech transcript still streams live into the composer; when the user stops the mic, `onend` sends the spoken text to a new `mode: "polish"` branch in `api/chat.js` (calls `polishTranscript()` on gemini-2.5-flash, thinkingBudget 0) which does a LIGHT-TOUCH cleanup: fixes punctuation/capitalisation/typos and removes filler words, but keeps the user's exact words and structure (no rephrasing). The cleaned text replaces the dictation in the input. Cleanup is best-effort: any failure returns the original text so dictation is never lost. Verified the example "...investing in uh apartment or whatever" -> "...investing in an apartment."
- [2026-06-27] Session 81: Added live web search to the general lane. Verified Google Search grounding works in the project (gemini-2.5-flash @ europe-west1 and gemini-3.5-flash @ global). Added `callGeminiVertexGrounded()` (googleSearch tool), rewrote `buildGeneralSystemPrompt()` to inject the current UTC date and instruct automatic search for time-sensitive questions + correct past/future time reasoning, split the general/other_city handler so only `general` searches, appended grounded sources as a markdown `Sources:` line, and extended `renderInline()` to render links. Validated end to end: "Spain's next World Cup game" searched and returned a dated, sourced answer; "capital of Japan" answered directly without searching.
- [2026-06-27] Session 80: Made Gemini 3.5 Flash the default selected model (first in the picker, `default` flag moved to it, `modelId` useState changed to `gemini-3.5-flash`). Backend `DEFAULT_MODEL` fallback intentionally left as `gemini-2.5-pro` (reliable europe-west1).
- [2026-06-27] Session 79: Added Gemini 3.5 Flash to the model picker and made it actually run. Probe found `gemini-3.5-flash` is 404 in `europe-west1` but 200 in the `global` region for this project, so added `MODEL_LOCATION_OVERRIDES` ({ "gemini-3.5-flash": "global", "gemini-flash-latest": "global" }) and routed each model to its required location in `callVertexModelWithFallback()` (fallback stays on the default region). Verified a real generation at the app's 2600-token budget returns clean text (`finishReason: STOP`; it spends ~414 thinking tokens first, so tiny budgets return empty).
- [2026-06-26] Session 78: Demo polish for the pending Claude quota. Added per-item `badge` support to the model picker (`aria-ui2.jsx`) and tagged the two Claude models with a "Soon" badge + "Pending Anthropic quota approval" desc (`aria-data.jsx`). Made `modelFallbackNotice()` provider-aware: Anthropic models now say "awaiting quota approval from Anthropic on Vertex AI" instead of the generic not-available text. Revert the badge/desc once quota is granted.
- [2026-06-26] Session 77: Ran a real Vertex model probe with a downloaded service-account key. Definitive results in `europe-west1`: only `gemini-2.5-pro`, `gemini-2.5-flash`, `gemini-2.5-flash-lite` return 200; `gemini-2.0-flash`/`gemini-3.x`/`-latest` are 404. `claude-sonnet-4-6` and `claude-opus-4-7` return 429 "Quota exceeded" (valid + enabled IDs, just no quota), so they were KEPT (my earlier swap to 4.5/4.1 was wrong — those 404). Trimmed the picker to the 3 working Gemini models + the 2 correct Claude IDs, made `isModelUnavailableError()` also treat 429/quota as a fallback trigger, updated the fallback note, and removed the temporary `?probe=models` endpoint. Claude needs an Anthropic quota grant in GCP to run for real.
- [2026-06-26] Session 76: Investigated why only Gemini 2.5 Pro/Flash produced output in the model picker. Confirmed via the production health endpoint that auth/connection works (`authConfigured:true`, project `capstoneprojectkpmg`, `europe-west1`), and that the dev preview deployment has NO env vars (`authConfigured:false`), so live calls and the probe only work on production. Replaced the invalid/aspirational model IDs (`gemini-3.5-flash`, `gemini-3.1-pro-preview`, `claude-sonnet-4-6`, `claude-opus-4-7`) with verified-valid Vertex IDs (Gemini 2.5 Flash-Lite, Gemini 2.0 Flash, Claude Sonnet 4.5 `@20250929`, Claude Opus 4.1 `@20250805`). Added `callVertexModelWithFallback()` (retry with default Gemini + transparent note) and a temporary `GET /api/chat?probe=models` diagnostic. Claude still needs per-model Model Garden enablement to run for real.
- [2026-06-26] Session 75: Tightened the upstream `isInScopeDomain()` gate after finding it fast-pathed ANY Paris/Athens mention into analytics (so "weather in Paris", "history of Athens", "is it safe to walk in Athens", "parts of a car engine" [parts~paris], "who is Aria", "is KPMG hiring" all produced investment briefs). Gate now requires unambiguous ARIA-internal terms, compliance, both cities, or city + real-estate/STR intent; removed aria/kpmg/prophet common-word triggers; switched the gate's city detection to exact aliases (fuzzy:false). Per product decisions: city + non-real-estate -> general (answer normally, note STR focus); city + real-estate incl. home-buying -> analytics. Strengthened the router prompt to match. Verified offline with VS Code's bundled Node (ELECTRON_RUN_AS_NODE): scope_smoke_test.mjs 24/24 pass, both API modules load clean. Estimated routing-logic quality ~97/100.
